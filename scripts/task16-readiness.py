...
s = p.read_text()
start = s.index('    private function prebid(Site $site, ResolvedSiteEngineState $engineState): array\n')
end = s.index('    private function native(Site $site, ResolvedSiteEngineState $engineState): array\n', start)
method = '''    private function prebid(Site $site, ResolvedSiteEngineState $engineState): array
    {
...
old = "                'engines' => $engineState->publicEngineState($connection?->network_code ?: ($engineState->gamRequired ? $site->current_gam_network_code : null)),\n"
new = old + "                'prebid_control' => [\n                    'configured_mode' => $engineState->prebidConfiguredMode->value,\n                    'resolved_mode' => $engineState->prebidDeliveryMode->value,\n                    'reason' => $engineState->prebidReason,\n                ],\n"
if old not in s:
