...
$dbMigrations = $exists('migrations') ? DB::table('migrations')->pluck('migration')->map(static fn ($value) => (string) $value)->all() : [];
...
foreach ($legalDocuments as $document) {
    if ((bool) ($document['required'] ?? false)) {
        $legalValid = $legalValid
            && str_starts_with((string) ($document['url'] ?? ''), 'https://')
            && trim((string) ($document['version'] ?? '')) !== '';
    }
}
...
// Mail / operational notifications. Emit booleans and transport names only; never credentials or addresses.
$mailer = strtolower(trim((string) config('mail.default')));
$mailFrom = trim((string) config('mail.from.address'));
$senderReady = ! $isPlaceholder($mailFrom) && filter_var($mailFrom, FILTER_VALIDATE_EMAIL) !== false;
$mailTransportReady = false;
$mailEvidence = ['mailer' => $mailer, 'sender_configured' => $senderReady];

if ($senderReady && ! in_array($mailer, ['', 'log', 'array'], true)) {
    if ($mailer === 'smtp') {
        $smtpHost = trim((string) config('mail.mailers.smtp.host'));
        $smtpUrl = trim((string) config('mail.mailers.smtp.url'));
        $hostReady = ! $isPlaceholder($smtpHost);
        $urlReady = $smtpUrl !== '' && str_contains($smtpUrl, '://') && ! $isPlaceholder($smtpUrl);
        $mailTransportReady = $hostReady || $urlReady;
        $mailEvidence['smtp_endpoint_configured'] = $mailTransportReady;
        ...
        $members = (array) config("mail.mailers.{$mailer}.mailers", []);
        $mailTransportReady = $members !== [];
        $mailEvidence['member_transport_count'] = count($members);
    } else {
        $mailTransportReady = (bool) config("mail.mailers.{$mailer}.transport");
        $mailEvidence['named_transport_configured'] = $mailTransportReady;
    }
}

$mailConfigured = $senderReady && $mailTransportReady;
$add('Notifications', 'production_mail', $mailConfigured ? 'PASS' : 'NOT_CONFIGURED', 'P1', $mailConfigured ? 'Production mail transport and sender are configured.' : 'Production mail transport or sender is missing/placeholder.', $mailEvidence);
...
// THOTH is deliberately advisory and never a Publisher/Site lifecycle blocker.
...
$thothProvider = strtoupper(trim((string) ($thothSetting?->active_provider ?: config('thoth.default_provider'))));
$providerRow = $exists('ai_provider_connections')
    ? DB::table('ai_provider_connections')->whereIn('provider', [$thothProvider, strtolower($thothProvider)])->orderByDesc('updated_at')->first()
    : null;
$providerCredential = (bool) ($providerRow && ! empty($providerRow->encrypted_credential));
$providerCredential = $providerCredential || filled(config('thoth.credentials.'.$thothProvider));
$providerFresh = (bool) ($providerRow
    && strtoupper((string) $providerRow->status) === 'CONNECTED'
    && $providerRow->last_connected_at
    && Carbon::parse((string) $providerRow->last_connected_at)->gte(now()->subMinutes((int) config('thoth.connection_max_age_minutes', 1440))));
...
$gamHealthyReal = $count('gam_connections', fn ($query) => $query->where('is_enabled', 1)->where('dry_run_default', 0)->where('health_status', 'HEALTHY')->whereNull('deleted_at'));
...
$demandApproved = $count('demand_accounts', fn ($query) => $query->where('is_enabled', 1)->where('approval_status', 'APPROVED')->whereNull('deleted_at'));
$demandTested = $count('demand_accounts', fn ($query) => $query->where('is_enabled', 1)->where('approval_status', 'APPROVED')->whereNotNull('last_tested_at')->whereNull('deleted_at'));
...
$prebidEnabled = $count('prebid_settings', fn ($query) => $query->where('enabled', 1));
...
$managedSellers = $count('seller_declarations', fn ($query) => $query->where('identity_source', 'HORUS_MANAGED'));
...
$reportEnabled = $count('report_source_connections', fn ($query) => $query->where('is_enabled', 1));
...
$currentTerms = $count('publisher_contracts', fn ($query) => $query
    ...
    ->where(fn ($nested) => $nested->whereNull('starts_at')->orWhereDate('starts_at', '<=', $today))
    ->where(fn ($nested) => $nested->whereNull('ends_at')->orWhereDate('ends_at', '>=', $today)));
$activeRules = $count('revenue_rules', fn ($query) => $query->whereNull('deleted_at')->where('is_active', 1));
...
// Advertiser scope is not a Publisher-pilot blocker when no approved/scheduled/active campaigns exist.
$campaignsEnabled = (bool) config('campaigns.advertiser_campaigns_enabled');
$activeCampaigns = $count('campaigns', fn ($query) => $query->whereIn('status', ['APPROVED', 'SCHEDULED', 'ACTIVE'])->whereNull('deleted_at'));
...
$add('Advertiser', 'creative_malware_scanner', $advertiserStatus, $activeCampaigns > 0 && ! $scannerReady ? 'P1' : 'P3', $activeCampaigns > 0 ? ($scannerReady ? 'Approved/scheduled/active advertiser scope has an executable malware scanner.' : 'Approved/scheduled/active advertiser campaigns exist without an executable malware scanner.') : ($campaignsEnabled ? 'Advertiser campaigns are enabled but none are approved/scheduled/active; scanner readiness is not a Publisher-pilot blocker.' : 'Advertiser campaigns are disabled.'), ['campaigns_enabled' => $campaignsEnabled, 'active_scope_campaigns' => $activeCampaigns, 'scanner_configured' => $scannerPath !== '', 'scanner_executable' => $scannerReady]);
