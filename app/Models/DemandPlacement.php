<?php

namespace App\Models;

use App\Enums\DemandApprovalStatus;
use App\Enums\DemandIntegrationMode;
use App\Enums\DemandSyncStatus;
use App\Models\Concerns\BelongsToOrganization;
use Illuminate\Database\Eloquent\Concerns\HasUlids;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\Relations\BelongsTo;
use Illuminate\Database\Eloquent\Relations\HasMany;

class DemandPlacement extends Model
{
    use BelongsToOrganization, HasUlids;

    protected $fillable = [
        'organization_id', 'demand_site_id', 'placement_id', 'approval_status',
        'is_enabled', 'integration_mode', 'fallback_priority', 'remote_placement_id',
        'placement_code', 'configuration', 'sync_status', 'last_synced_at',
        'created_by', 'updated_by',
    ];

    protected function casts(): array
    {
        return [
            'approval_status' => DemandApprovalStatus::class,
            'integration_mode' => DemandIntegrationMode::class,
            'sync_status' => DemandSyncStatus::class,
            'is_enabled' => 'boolean',
            'configuration' => 'array',
            'last_synced_at' => 'datetime',
        ];
    }

    public function demandSite(): BelongsTo { return $this->belongsTo(DemandSite::class); }
    public function placement(): BelongsTo { return $this->belongsTo(Placement::class); }
    public function widgets(): HasMany { return $this->hasMany(DemandWidget::class); }
}
