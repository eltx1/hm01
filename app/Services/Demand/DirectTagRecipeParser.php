...
final class DirectTagRecipeParser
{
     ...
     *
     * Structural shape is reported as data, not as a security warning. A
     * provider connector decides whether it requires one container, no
     * container, or an isolated multi-element fragment.
     *
    ...
    public function parse(string $markup): array
    {
        ...
        if ($markup === '') return $this->result([], [], [], ['The supplied tag is empty.'], true);
        if (strlen($markup) > 100_000) return $this->result([], [], [], ['The supplied tag exceeds the safe import size.'], true);
        if (preg_match('/javascript\s*:/i', $markup)) $warnings[] = 'javascript: URLs are not permitted.';
        ...
        if (preg_match('/<\s*(?:object|embed|base|meta)\b/i', $markup)) $warnings[] = 'Unsupported active HTML element detected.';
        if (preg_match('/\bon[a-z]+\s*=/i', $markup)) $warnings[] = 'Inline event-handler attributes are not permitted.';
        ...
        foreach ($scriptMatches as $match) {
            ...
            if ($src !== '') {
                ...
                if (! $this->httpsUrl($src)) $warnings[] = 'Every external script must use an absolute HTTPS URL.';
                $scripts[] = ['url' => $src, 'async' => array_key_exists('async', $attributes), 'defer' => array_key_exists('defer', $attributes), 'attributes' => $this->publicAttributes($attributes)];
            }
            if ($body !== '') $inline[] = $body;
        }
        ...
        foreach ($elementMatches as $match) {
            ...
            if (in_array($element, ['script', 'html', 'head', 'body'], true)) continue;
            ...
            $containers[] = ['element' => $element, 'id' => $id, 'class' => $class, 'attributes' => $this->publicAttributes($attributes)];
        }

        foreach ($scripts as $script) {
            if (preg_match(self::SENSITIVE, json_encode($script, JSON_UNESCAPED_SLASHES) ?: '')) {
                $sensitive = true; $warnings[] = 'Secret-like script attributes are not permitted.'; break;
            }
        }
        foreach ($containers as $container) {
            if (preg_match(self::SENSITIVE, json_encode($container, JSON_UNESCAPED_SLASHES) ?: '')) {
                $sensitive = true; $warnings[] = 'Secret-like container attributes are not permitted.'; break;
            }
        }
    ...
    {
        return ['detectedScripts' => $scripts, 'detectedContainers' => $containers, 'detectedPublicIdentifiers' => $this->identifiers($containers), 'unsupportedInlineCode' => array_map(fn (string $code): string => mb_substr(preg_replace('/\s+/', ' ', $code) ?: '', 0, 240), $inline), 'inlineCode' => $inline, 'securityWarnings' => $warnings, 'containsSensitiveMaterial' => $sensitive];
    }
    ...
    private function publicAttributes(array $attributes): array
    {
        ...
        foreach ($attributes as $key => $value) {
            $key = strtolower((string) $key); $value = trim((string) $value);
            if ($key === '' || str_starts_with($key, 'on') || preg_match(self::SENSITIVE, $key)) continue;
            if (! preg_match('/^(?:data-[a-z0-9_.:-]+|aria-[a-z0-9_.:-]+|type|crossorigin|referrerpolicy|async|defer)$/', $key)) continue;
            if (preg_match('/javascript\s*:/i', $value) || preg_match('/^(?:env|file):/i', $value)) continue;
            $safe[$key] = $value;
        }
        return $safe;
    }
    ...
    private function identifiers(array $containers): array
    {
        ...
        foreach ($containers as $container) {
            foreach (['id', 'data-widget-id', 'data-zone-id', 'data-zoneid', 'data-placement-id'] as $key) {
                ...
                if (is_string($value) && $value !== '') $ids[] = $value;
            }
        }
