...
const css = (await Promise.all(['brand-tokens', 'components', 'app', 'ux-launch', 'interface-density', 'dashboard-theme', 'reporting-experience', 'form-experience'].map(name => readFile(new URL(`resources/css/${name}.css`, root), 'utf8')))).join('\n').replace(/^@import .*;$/gm, '');
