...
class PremiumPublisherApplicationExperienceTest extends TestCase
{
    ...
    public function test_turnstile_requires_server_validation_and_rejects_invalid_expired_and_replayed_tokens(): void
    {
        ...
        $this->withServerVariables(['REMOTE_ADDR' => '203.0.113.5'])
            ->post(route('publisher-registration.store'), $this->registrationPayload([
                ...
                'primary_domain' => 'second-publisher.example',
    ...
    public function test_applicant_can_save_each_decision_step_resume_and_keep_validation_input(): void
    {
        ...
        $this->from(route('publisher-application.show', ['step' => 2]))
            ->put(route('publisher-application.update'), [
                'step' => 2, 'contact_name' => '', 'legal_name' => 'Publisher LLC',
                'publisher_name' => 'Publisher', 'primary_domain' => 'publisher.example',
            ])->assertSessionHasErrors('contact_name');
        $this->get(route('publisher-application.show', ['step' => 2]))
            ->assertSee('role="alert"', false)
            ->assertSee('Publisher LLC');

        $this->put(route('publisher-application.update'), $this->websiteStepPayload())
            ->assertRedirect(route('publisher-application.show', ['step' => 3]));
        ...
        $this->post(route('logout'));
        $this->post(route('login.store'), ['email' => $user->email, 'password' => 'Secure-Password-2026!'])
            ->assertRedirect(route('publisher-application.show'));
        $this->get(route('publisher-application.show', ['step' => 3]))
            ->assertOk()
            ->assertSee('Original independent reporting and analysis.')
    ...
    public function test_legal_acceptance_is_version_exact_historical_and_marketing_is_independent(): void
    {
        ...
        $this->post(route('publisher-application.submit'), ['confirm' => 1])
            ->assertSessionHasErrors(['legal.TERMS_OF_SERVICE', 'legal.PRIVACY_POLICY']);
        ...
        $this->assertDatabaseHas('publisher_application_marketing_consents', [
            'user_id' => $user->id, 'opted_in' => 0,
        ]);
        ...
        $this->assertSame(1, PublisherApplicationLegalAcceptance::query()
            ->where('document_type', 'TERMS_OF_SERVICE')->where('document_version', '2026-08')->count());
        $this->assertDatabaseMissing('publisher_application_legal_acceptances', [
            'document_type' => 'TERMS_OF_SERVICE', 'document_version' => '2026-09',
        ]);
    ...
    public function test_verification_received_and_lifecycle_emails_use_branded_application_communications(): void
    {
        ...
        Mail::assertSent(HorusNotificationMail::class, fn (HorusNotificationMail $mail) =>
            $mail->hasTo($user->email) && $mail->item->type === 'PUBLISHER_APPLICATION_RECEIVED');
    }
    ...
    public function test_status_page_hides_internal_review_metadata_and_admin_sees_review_evidence(): void
    {
        ...
        $this->get(route('publisher-application.show', ['step' => 5]))
            ->assertOk()
            ->assertSee('THOTH advisory cannot approve or reject')
            ->assertDontSee('internal risk score')
        ...
        $this->actingAs($admin)->withSession(['two_factor_passed_at' => now()->timestamp])
            ...
            ->assertOk()
            ->assertSee('Publisher quality evidence')
            ->assertSee('Legal documents &amp; consent', false)
            ->assertSee('Terms Of Service')
            ->assertSee('2026-08')
            ->assertSee('Optional marketing consent')
            ->assertSee('THOTH may append advisory evidence but can never approve or reject');
    }
