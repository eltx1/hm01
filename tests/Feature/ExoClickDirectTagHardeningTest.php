...
final class ExoClickDirectTagHardeningTest extends TestCase
{
    ...
    public function test_exoclick_recipe_survives_public_delivery_sanitization(): void
    {
        $this->seedIdentity();
        $this->seed(DemandNetworkSeeder::class);
        ...
        $account = app(DemandAccountService::class)->create([
            'organization_id' => $horus->id,
            'demand_network_id' => $network->id,
            'name' => 'ExoClick public config test',
            'scope' => DemandAccountScope::HorusMedia,
            'integration_mode' => DemandIntegrationMode::DirectJs,
            'approval_status' => DemandApprovalStatus::Approved,
            'is_enabled' => true,
        ...
        $script = '<script async type="application/javascript" src="https://a.magsrv.com/ad-provider.js"></script>';
        $queue = '<script>(AdProvider = window.AdProvider || []).push({"serve": {}});</script>';
        $review = $connector->parseDirectTag($script.'<ins class="TEST_PROVIDER_CLASS" data-zoneid="TEST_ZONE_ID"></ins>'.$queue);

        $this->assertTrue($review['safe']);
        $this->assertIsArray($review['recipe']);

        $builder = (new ReflectionClass(DemandConfigurationBuilder::class))->newInstanceWithoutConstructor();
        $sanitize = new ReflectionMethod(DemandConfigurationBuilder::class, 'sanitizePublicTag');
        $sanitize->setAccessible(true);
        $public = $sanitize->invoke($builder, $review['recipe']);

        $this->assertSame('ins', data_get($public, 'container.element'));
        $this->assertSame('TEST_PROVIDER_CLASS', data_get($public, 'container.class'));
        $this->assertSame('TEST_ZONE_ID', data_get($public, 'container.attributes.data-zoneid'));
        $this->assertSame('EXOCLICK_SERVE', data_get($public, 'initialization.type'));
        $this->assertSame('https://a.magsrv.com/ad-provider.js', data_get($public, 'scripts.0.url'));
    }
}
