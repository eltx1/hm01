...
class GamRestConnectorTest extends TestCase
{
    ...
    public function test_unpublished_rest_capability_never_falls_back_to_soap(): void
    {
        $this->seedIdentity();
        $organization = $this->makeOrganization(OrganizationType::HorusMedia);
        $actor = $this->makeUser($organization, RoleName::SuperAdmin);
        ...
        $result = app(GamConnectorManager::class)->for($connection)->createCreative(['displayName' => 'Creative']);

        $this->assertFalse($result->success);
        $this->assertSame('REST_CAPABILITY_UNAVAILABLE', $result->errorCode);
        Http::assertNothingSent();
    }
