...
class AdvertiserCampaignCapabilityGuardrailTest extends TestCase
{
    ...
    public function test_gamless_publisher_modes_do_not_become_advertiser_delivery_backends(): void
    {
        foreach ([ServingMode::HorusDirect, ServingMode::StandalonePrebid, ServingMode::DirectJs] as $mode) {
            $fx = $this->fixture();
            $fx['site']->update(['serving_mode' => $mode, 'gam_connection_id' => null]);

            $result = app(CampaignDeliveryCapabilityService::class)->evaluate($fx['campaign']->fresh(), true);

            $this->assertSame(CampaignDeliveryCapabilityStatus::NoGamBackend, $result->status, $mode->value);
            $this->assertSame($mode, $fx['site']->fresh()->serving_mode);
        }
    }
    ...
    public function test_wrong_connection_type_and_missing_remote_mapping_are_not_treated_as_available(): void
    {
        $wrong = $this->fixture();
        $wrong['site']->update(['serving_mode' => ServingMode::McmPartnerGam]);
        $this->assertSame(
            CampaignDeliveryCapabilityStatus::ConfigurationIncomplete,
            app(CampaignDeliveryCapabilityService::class)->evaluate($wrong['campaign'], true)->status,
        );
    ...
    public function test_feature_off_blocks_new_creation_and_submission_but_keeps_existing_campaign_readable(): void
    {
        ...
        try {
            app(CampaignWorkflowService::class)->create($fx['advertiser'], [], $fx['advertiser_user']);
        ...
        try {
            app(CampaignWorkflowService::class)->submit($fx['campaign']->fresh(), $fx['advertiser_user']);
        ...
        $this->assertSame($fx['campaign']->id, $fx['advertiser']->campaigns()->withoutGlobalScopes()->findOrFail($fx['campaign']->id)->id);
    }

    public function test_draft_can_be_saved_without_backend_but_cannot_be_submitted(): void
    {
        $fx = $this->fixture();
        $fx['site']->update(['serving_mode' => ServingMode::DirectJs, 'gam_connection_id' => null]);
    ...
    public function test_admin_approval_and_scheduling_cannot_bypass_lost_capability(): void
    {
        ...
        $submitted = $workflow->submit($approval['campaign'], $approval['advertiser_user']);
        ...
        $submitted = $workflow->submit($scheduling['campaign'], $scheduling['advertiser_user']);
    ...
    public function test_customer_projection_hides_gam_identity_while_admin_projection_contains_exact_backend_details(): void
    {
        ...
        $customer = json_encode($result->customerProjection(), JSON_THROW_ON_ERROR);
        $admin = $result->internalProjection();
    ...
    public function test_feature_setting_is_permission_protected_audited_and_delivery_warning_is_deduplicated(): void
    {
        $fx = $this->fixture();
        ...
        $this->actingAs($publisherUser)
            ->put('/admin/settings/advertiser_campaigns.enabled', ['value' => false])
            ->assertForbidden();
    ...
    {
        $workflow = app(CampaignWorkflowService::class);
        $submitted = $workflow->submit($fx['campaign']->fresh(), $fx['advertiser_user']);
    ...
    private function fixture(): array
    {
        ...
        $adUnit = $inventory->createAdUnit($site, [
            'name' => 'Top', 'code' => 'task41-top',
            'sizes' => [['width' => 300, 'height' => 250]],
        ], $admin);
        $placement = $inventory->createPlacement($site, [
            'name' => 'Top', 'code' => 'task41-top', 'type' => 'DISPLAY', 'status' => 'ACTIVE',
        ...
        $accounts->saveBillingProfile($advertiser, [
            'legal_name' => 'Task 41 Advertiser LLC', 'billing_email' => 'billing-task41@advertiser.test',
        ...
        $campaign = $workflow->create($advertiser, [
            'name' => 'Task 41 capability campaign', 'objective' => 'Awareness', 'pricing_model' => 'CPM',
            ...
            'site_ids' => [$site->id], 'placement_ids' => [$placement->id],
            'frequency_cap_impressions' => 3, 'frequency_cap_days' => 1,
            'landing_url' => 'https://advertiser.test/task41',
        ], $advertiserUser);
        $creative = app(CampaignCreativeService::class)->create($campaign, [
            'name' => 'Task 41 text creative', 'type' => 'TEXT', 'text_content' => 'Task 41 offer',
