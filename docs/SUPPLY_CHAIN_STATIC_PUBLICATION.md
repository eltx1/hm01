...
The production main-site Worker owns the canonical route and redirects it directly to the authoritative CDN artifact. It never keeps a second sellers.json copy. Every eligible Seller, Publisher, Site, ownership, mapping, or ads.txt mutation queues the global supply-chain artifact; the scheduler or an authorized manual deploy publishes a complete replacement snapshot. The canonical URL therefore follows each confirmed CDN deployment automatically without a separate main-website release.
