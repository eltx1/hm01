...
@section('navigation')
@foreach(['Campaigns','Creatives','Billing','Reports'] as $item)<span class="{{ $loop->first ? 'active' : '' }}">{{ $item }}</span>@endforeach<a href="{{ route('account.branding.edit') }}">Account settings</a>
@endsection
@section('content')
