...
<section class="import-report" aria-labelledby="ads-txt-import-report-heading">
    <div class="section-heading">
        <div>
            ...
            <h3 id="ads-txt-import-report-heading">{{ $report['created'] ?? 0 }} created · {{ $report['skipped'] ?? 0 }} existing · {{ $report['invalid_total'] ?? count($report['invalid'] ?? []) }} invalid</h3>
        </div>
    ...
    <p class="muted">{{ $report['total_lines'] ?? 0 }} input lines · {{ $report['ignored'] ?? 0 }} comments/directives ignored · {{ $report['duplicates'] ?? 0 }} duplicate input rows skipped.</p>
