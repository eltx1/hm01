...
<x-control-plane.workspace-tabs :items="[
    ['label' => 'Status', 'href' => '#publisher-site-overview'],
    ['label' => 'Monetization', 'href' => '#monetization-health'],
...
<article id="publisher-ads-txt" class="workspace-section publisher-ads-installation">
...
@endif
<section id="publisher-site-setup" class="detail-grid">
...
<article id="publisher-domain-verification"><div class="section-heading"><div><p class="eyebrow">Authorized domains</p><h2>Ownership verification</h2></div></div>
...
<article id="publisher-review-status"><p class="eyebrow">Review</p><h2>Submission status</h2>@forelse($site->reviews->sortByDesc('created_at') as $review)<div class="event"><div><strong>{{ $review->decision }}</strong><p>{{ $review->publisher_message }}</p></div><span>{{ $review->created_at }}</span></div>@empty<p class="muted">Waiting for successful ads.txt verification; submission is automatic.</p>@endforelse</article>
@endif
