...
<article class="workspace-section">
    ...
    @forelse($records as $record)
        ...
        @if(auth()->user()->hasPermission('supply_chain.ads_txt.manage'))<tr><td colspan="6"><details><summary>Advanced single-record controls</summary><form method="POST" action="{{ route('admin.compliance.ads-txt.master.update', $record) }}" class="form-stack">@csrf @method('PUT')<div class="form-grid"><label>Domain<input class="hm-input" name="advertising_system_domain" value="{{ $record->advertising_system_domain }}" required></label><label>Seller ID<input class="hm-input" name="publisher_account_id" value="{{ $record->publisher_account_id }}" required></label><label>Relationship<select class="hm-input" name="relationship"><option @selected($record->relationship==='DIRECT')>DIRECT</option><option @selected($record->relationship==='RESELLER')>RESELLER</option></select></label><label>Authority ID<input class="hm-input" name="certification_authority_id" value="{{ $record->certification_authority_id }}"></label><label>Effective from<input class="hm-input" type="datetime-local" name="effective_from" value="{{ $record->effective_from?->format('Y-m-d\TH:i') }}"></label><label>Effective to<input class="hm-input" type="datetime-local" name="effective_to" value="{{ $record->effective_to?->format('Y-m-d\TH:i') }}"></label></div><label>Internal notes<textarea class="hm-input" name="internal_notes">{{ $record->internal_notes }}</textarea></label><button class="hm-button-secondary">Save and reopen review</button></form>
            @if($record->status === 'ACTIVE')<form method="POST" action="{{ route('admin.compliance.ads-txt.master.disable', $record) }}" class="form-stack">@csrf<label>Reason<textarea class="hm-input" name="reason" required minlength="8"></textarea></label><label>Current password<input class="hm-input" type="password" name="current_password" required></label><label>Type <code>DISABLE {{ $impactedSiteCount }} SITES</code><input class="hm-input" name="impact_confirmation" required></label><button class="hm-button-danger">Disable globally</button></form>@elseif(auth()->user()->hasPermission('supply_chain.sellers.review'))<form method="POST" action="{{ route('admin.compliance.ads-txt.master.enable', $record) }}" class="form-stack">@csrf<label>Reason<textarea class="hm-input" name="reason" required minlength="8"></textarea></label><label>Current password<input class="hm-input" type="password" name="current_password" required></label><label>Type <code>ENABLE {{ $impactedSiteCount }} SITES</code><input class="hm-input" name="impact_confirmation" required></label><button class="hm-button-primary">Enable globally</button></form>@endif</details></td></tr>@endif
