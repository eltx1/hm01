...
<article id="reporting" class="workspace-section">
    @if($todayReport ?? null)
        <section id="today-report" aria-labelledby="today-report-heading">
            ...
            @if(auth()->user()->hasPermission('reporting.admin.view'))
                <a class="hm-button-secondary button-link" href="{{ route('admin.reporting.index') }}">View completed-day reports</a>
            @endif
        </section>
    @endif
    ...
    @if($reportBinding)
        ...
        <dl><dt>Reporting currency</dt><dd>{{ $reportBinding->connection->currency }} · canonical Horus reporting</dd><dt>Google network currency</dt><dd>{{ data_get($reportBinding->connection->configuration, 'network_currency', 'Unknown') }} · metadata only</dd><dt>Reporting timezone</dt><dd>{{ $reportBinding->connection->timezone }}</dd><dt>Last successful import</dt><dd>{{ $reportBinding->connection->last_successful_import_at ?? 'Waiting for the first automatic synchronization' }}</dd><dt>Last finalized import</dt><dd>{{ $reportBinding->connection->last_finalized_import_at ?? 'Pending' }}</dd></dl>
    ...
    @if($reportingGamConnections->isNotEmpty())
        <form method="POST" action="{{ route('admin.sites.reporting.gam.store', $site) }}" class="form-stack" data-gam-report-binding data-units-url="{{ route('admin.sites.reporting.gam.units', $site) }}">
            ...
            <p class="muted">Reports cover the selected unit only, excluding child units. Horus always requests revenue from Google in US Dollar (USD); the Google network currency is retained only as source metadata. The reporting timezone still comes from Google. The first import covers the current open month, starting after any days already imported for this website. Existing closed financial history is never rewritten. Ad delivery settings stay as configured.</p>
