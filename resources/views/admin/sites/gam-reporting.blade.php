...
<article id="reporting" class="workspace-section">
    ...
    @if($reportBinding)
        ...
        <dl><dt>Horus reporting currency</dt><dd><strong>{{ $reportBinding->connection->currency }}</strong> · fixed platform reporting currency</dd><dt>Google network currency</dt><dd>{{ data_get($reportBinding->connection->configuration, 'network_currency', 'Unknown') }} · source metadata only</dd><dt>Reporting timezone</dt><dd>{{ $reportBinding->connection->timezone }}</dd><dt>Last successful import</dt><dd>{{ $reportBinding->connection->last_successful_import_at ?? 'Waiting for the first automatic synchronization' }}</dd><dt>Last finalized import</dt><dd>{{ $reportBinding->connection->last_finalized_import_at ?? 'Pending' }}</dd></dl>
    ...
    @if($reportingGamConnections->isNotEmpty())
        <form method="POST" action="{{ route('admin.sites.reporting.gam.store', $site) }}" class="form-stack" data-gam-report-binding data-units-url="{{ route('admin.sites.reporting.gam.units', $site) }}">
            ...
            <p class="muted">Reports cover the selected unit only, excluding child units. Revenue is requested from Google directly in USD regardless of the Ad Manager network currency; the reporting timezone still comes from Google. The first import covers the current open month, starting after any locked historical period. Existing finalized history is preserved. Ad delivery settings stay as configured.</p>
            <button class="hm-button-primary">{{ $reportBinding ? 'Update reporting connection' : 'Connect reports' }}</button>
        </form>
        <p class="muted">Synchronization checks every five minutes. Today's daily totals are estimates refreshed every hour; completed days refresh every six hours while the financial period is open. Existing non-USD Site GAM connections automatically cut over to USD safely on their next synchronization. Google may take time to prepare a report.</p>
    @endif
</article>
