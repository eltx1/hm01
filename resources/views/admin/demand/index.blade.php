        ...
        @foreach($networks as $network)
            <details class="domain-card">
                ...
                <form class="form-grid" method="POST" action="{{ route('admin.demand.networks.settings', $network) }}">
                    ...
                    <label>Formats<select class="hm-input" name="supported_formats[]" multiple>@foreach(['DISPLAY','NATIVE','VIDEO','OUTSTREAM','REWARDED'] as $format)<option value="{{ $format }}" @selected(in_array($format, data_get($network->capabilities, 'supported_formats', []), true))>{{ $format }}</option>@endforeach</select></label>
