...
<form class="form-grid" method="POST" action="{{ $editing ? route('admin.gam.connections.update', $connection) : route('admin.gam.connections.store') }}">
    ...
    <label>Connector driver<select class="hm-input" name="driver"><option value="HYBRID" selected>REST-first · official SOAP fallback</option></select><small class="muted">REST v1 is primary. Only capabilities Google has not published in REST use the newest installed official SOAP library.</small></label>
