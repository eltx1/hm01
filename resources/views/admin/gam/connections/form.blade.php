...
<form class="form-grid" method="POST" action="{{ $editing ? route('admin.gam.connections.update', $connection) : route('admin.gam.connections.store') }}">
    ...
    <label>Connector driver<select class="hm-input" name="driver"><option value="REST" selected>GAM REST v1 · version-stable</option></select><small class="muted">No dated SOAP version and no silent SOAP fallback.</small></label>
