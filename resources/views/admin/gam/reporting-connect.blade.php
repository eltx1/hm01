...
@section('content')
<section class="hero"><div><p class="eyebrow">{{ $site->display_name }}</p><h2>Connect your Google account</h2><p>Choose your Google account, then the ad unit for this website. You can connect several Google accounts and Ad Manager networks.</p><p class="muted">All Horus financial reports are requested from Google in <strong>{{ strtoupper((string) config('reporting.canonical_currency', 'USD')) }}</strong>. The network currency shown below is source metadata only.</p></div><a class="hm-button-secondary button-link" href="{{ route('admin.sites.show', $site) }}#reporting">Back to website</a></section>
...
<article class="workspace-section">
    ...
    <form method="POST" action="{{ route('admin.sites.reporting.accounts.connect', $site) }}" class="form-stack">
        ...
        <label class="compact-row"><span><input type="{{ $adUnit !== '' ? 'radio' : 'checkbox' }}" name="networks[]" value="{{ $network['code'] }}" @checked(in_array($network['code'], old('networks', [])))> <strong>{{ $network['name'] }}</strong></span><span>{{ $network['code'] }} · network {{ $network['currency'] }} · reports {{ strtoupper((string) config('reporting.canonical_currency', 'USD')) }} · {{ $network['timezone'] }}</span></label>
        @endforeach
