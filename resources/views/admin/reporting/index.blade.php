...
@section('content')
<section class="hero"><div><p class="eyebrow">Reporting &amp; Revenue</p><h2>One USD view across connected GAM networks</h2><p>Horus requests GAM revenue from Google in USD even when the source network uses another base currency. Source currency remains provenance only; dashboards and new financial reporting use {{ $summary['currency'] }}.</p></div>@if(auth()->user()->hasPermission('finance.operations.view'))<a class="hm-button-primary button-link" href="{{ route('admin.finance.overview') }}">Open Finance Operations</a>@endif</section>
<form method="get" class="form-grid"><label>From<input type="date" name="from" value="{{ $summary['from']->toDateString() }}"></label><label>To<input type="date" name="to" value="{{ $summary['to']->toDateString() }}"></label><label>Reporting currency<input class="hm-input" value="{{ $summary['currency'] }}" readonly aria-readonly="true"><span class="muted">Horus requests GAM revenue from Google in USD regardless of the Ad Manager network currency.</span></label><button class="hm-button-primary" type="submit">Update dates</button></form>
