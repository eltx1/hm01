...
<body>
    ...
    <div class="applicant-shell">
        <header class="applicant-topbar">
            ...
            <div>@if(config('publisher-applications.support_url'))<a class="text-link" href="{{ config('publisher-applications.support_url') }}">Need help?</a>@endif<a class="text-link" href="{{ route('account.index') }}">My account</a><span>{{ auth()->user()->email }}</span><form method="POST" action="{{ route('logout') }}">@csrf<button class="text-button" type="submit">Sign out</button></form></div>
        </header>
